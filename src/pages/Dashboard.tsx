...
const Dashboard = () => {
  ...
  return (
    <div className="space-y-4">
      {/* Greeting */}
      <div>
        <h2 className="text-xl font-bold">{getGreeting()}, {profile.name}</h2>
        <p className="text-sm text-muted-foreground">{format(new Date(), 'EEEE, MMMM d')}</p>
        {totalDays > 0 && (
          <p className="text-xs text-muted-foreground mt-1">{totalDays} days of data. Your dashboard is getting interesting.</p>
        )}
      </div>
